    ...
    <section id="contact" className="py-24 px-6 md:px-16 lg:px-24 bg-[#0D1528] text-center">
      <div ref={ref} className="max-w-xl mx-auto">
        ...
        <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: .5 }}
          className="font-mono text-xs text-[#00CFDA] tracking-widest mb-2">// contact</motion.p>
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: .5, delay: .1 }}
          ...
          Let's <span className="text-[#4A8BF4]">Connect</span>
        </motion.h2>

        <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: .5, delay: .2 }}
          className="text-[#7A8BAD] font-light mb-10">
          Open to Gen AI / ML Engineering roles. Let's build something great together.
        </motion.p>

        <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: .5, delay: .3 }}
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: .5, delay: .4 }}
        ...
        {/* Social links */}
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: .5, delay: .5 }}
...
export function Footer() {
  return (
    <footer className="bg-[#060C1A] border-t border-[rgba(74,139,244,0.1)] px-6 md:px-16 lg:px-24 py-6
      ...
      <p className="font-mono text-xs text-[#7A8BAD]">© 2026 Annem Siva Sai Teja Reddy</p>
      <p className="font-mono text-xs text-[#4A8BF4]">Gen AI Developer · Hyderabad</p>
    </footer>
  )
}
