    ...
    <section id="hero" className="relative min-h-screen flex flex-col justify-center px-6 md:px-16 lg:px-24 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl">
        ...
        <motion.h1 custom={1} variants={fadeUp} initial="hidden" animate="show"
          ...
          <span className="block text-[#E8EDF8]">Annem Siva</span>
          <span className="block gradient-text">Sai Teja Reddy</span>
        </motion.h1>

        {/* Typewriter */}
        <motion.div custom={2} variants={fadeUp} initial="hidden" animate="show"
          className="flex items-center gap-3 font-mono text-base text-[#7A8BAD] mt-5 mb-5">
          <span className="text-[#4A8BF4]">&gt;</span>
          <Typewriter />
        </motion.div>
        ...
        <motion.p custom={3} variants={fadeUp} initial="hidden" animate="show"
          ...
          Building <strong className="text-[#E8EDF8] font-medium">production-grade Gen AI systems</strong> — RAG pipelines,
          agentic orchestration &amp; LLM-powered applications. Grounded in{' '}
          <strong className="text-[#E8EDF8] font-medium">3+ years</strong> of enterprise software engineering.
        </motion.p>
        ...
        <motion.div custom={5} variants={fadeUp} initial="hidden" animate="show"
          ...
          {[
            ...
            { val: '5+', label: 'AI Projects Built' },
            { val: '2',  label: 'Certifications' },
          ].map(s => (
            <div key={s.label}>
              <span className="font-display font-bold text-3xl text-[#E8EDF8]">
                {s.val.replace(/\d+/, m => ``)}<span className="text-[#4A8BF4]">{s.val}</span>
              </span>
              <p className="font-mono text-xs text-[#7A8BAD] mt-0.5">{s.label}</p>
            </div>
          ))}
        </motion.div>
      </div>
